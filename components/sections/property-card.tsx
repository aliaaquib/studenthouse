  ...
  return (
    <motion.div whileHover={{ y: compact ? -2 : -4 }} transition={{ duration: 0.22 }}>
      ...
      {property.popular && !compact ? (
        <div className="absolute left-4 top-4 flex h-8 items-center gap-1 rounded-full bg-[var(--primary)] px-3 text-[11px] font-extrabold leading-4 tracking-[0.4px] text-white shadow-[0_10px_28px_rgba(23,166,115,0.22)] dark:text-[#071411]">
          <Sparkles size={13} fill="currentColor" /> STUDENT PICK
        </div>
      ) : null}
      {property.verified && !compact ? (
        <div className="absolute right-4 top-4 flex h-8 items-center gap-1 rounded-full bg-white/92 px-3 text-[11px] font-extrabold text-[var(--primary)] shadow-[0_8px_22px_rgba(16,32,28,0.12)] backdrop-blur">
      ...
      <div className={compact ? "p-4 pt-3" : "p-5 pt-4 sm:p-6 sm:pt-5"}>
        <p className={compact ? "text-[15px] font-extrabold leading-[1.4] text-[var(--primary)]" : "text-[22px] font-extrabold leading-[1.35] text-[var(--primary)]"}>
          {property.price}
          <span className={compact ? "ml-1 text-[11px] font-medium text-[var(--muted)]" : "ml-1 text-[13px] font-medium text-[var(--muted)]"}>/month</span>
        </p>
        <h3 className={compact ? "mt-1 text-[14px] font-extrabold leading-[1.35]" : "mt-2 text-[19px] font-extrabold leading-[1.35]"}>
          {property.name}
        </h3>
        <p className={compact ? "mt-1 flex items-center gap-1.5 text-[11px] font-semibold leading-[1.35] text-[var(--muted)]" : "mt-2 flex items-center gap-1.5 text-[13px] font-semibold leading-[1.5] text-[var(--muted)]"}>
          <MapPin size={compact ? 13 : 15} /> {property.distance}
        </p>
        <div className={compact ? "my-3 h-px bg-[var(--border)]" : "my-4 h-px bg-[var(--border)]"} />
        <div className={compact ? "grid grid-cols-2 gap-2 text-[11px] font-bold text-[var(--muted-strong)]" : "grid grid-cols-2 gap-2 text-[12px] font-bold text-[var(--muted-strong)] sm:gap-3"}>
        ...
        {!compact ? <div className="mt-4 flex flex-wrap gap-2">{property.badges.slice(0, 3).map((badge) => <span key={badge} className="rounded-full bg-[var(--surface)] px-3 py-1 text-[11px] font-bold text-[var(--muted-strong)]">{badge}</span>)}</div> : null}
        {!compact ? (
          <a
            href={getWhatsAppHref(property)}
            target="_blank"
            rel="noreferrer"
            className="focus-ring mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-[12px] bg-[var(--primary)] px-4 text-[13px] font-extrabold text-white transition hover:-translate-y-0.5 hover:bg-[var(--primary-light)] dark:text-[#071411]"
          >
